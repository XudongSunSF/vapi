 ...
 * - `LibException`: Captures library-specific exceptions, locks the handle for writing,
 *   retrieves troubleshooting information, and returns an error with detailed context.
 * - `std::exception`: Captures standard exceptions, locks the handle for writing, and
 *   returns an error with the exception message.
 *
...
namespace staging
{
	...
	struct RatesData
	{
		RatePathsSet<PrimaryRateType> ratePaths_;
		RatePathsSet<KeyRate> keyRatePaths_;
	};
