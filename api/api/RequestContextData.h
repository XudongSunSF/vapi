 ...
 *
 * Invariant: modelConfigHandle_ MUST be the handle under which
 * *modelConfig_ is registered in the HandleContainer. Both fields describe
 * the same Tier 1 model config; the handle is what crosses the Java/native
 ...
 * request-level handles are created after the model-config handle they refer
 * to, LIFO destruction pops them first, so dependent handles are gone before
 * their model-config handle is destroyed. If the teardown order ever becomes
 * unordered, deleting a model-config handle (deleteHandle) must tolerate (or
 * reject) request contexts that still reference it.
 */
struct RequestContextData
{
    Handle modelConfigHandle_{};
    std::shared_ptr<const ModelConfigData> modelConfig_;
    std::variant<std::monostate, RequestContextBytes, RequestContextHandles> payload_;

    RequestContextData() = default;

    RequestContextData(
        Handle modelConfigHandle,
        std::shared_ptr<const ModelConfigData> modelConfig,
        RequestContextBytes bytes)
        : modelConfigHandle_(modelConfigHandle)
