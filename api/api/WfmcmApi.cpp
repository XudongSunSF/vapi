...
#include <src/app-common/messages/responses/GenRatePathsForMortgageValuationResponse.h>
    ...
    int contentLen)
{
    ...
    WRITE_LOCK_HANDLE(request)
    auto hashNum = std::hash<std::string_view>{}(std::string_view(content, contentLen));
    switch (handleData(request).getType())
    {
        case HandleType::CalcValueForMortgageRequest:
        {
            auto& indexMapper = handleObject<HandleType::CalcValueForMortgageRequest>(request).indexMapper_;
            indexMapper = IndexMapperReader::readFromCsv({ content, (size_t)contentLen });
            indexMapper.setHash(hashNum);
            break;
        }
        case HandleType::CalcValueForMortgageFromRatesRequest:
        {
            auto& indexMapper = handleObject<HandleType::CalcValueForMortgageFromRatesRequest>(request).indexMapper_;
            indexMapper = IndexMapperReader::readFromCsv({ content, (size_t)contentLen });
            indexMapper.setHash(hashNum);
            break;
        }
...
// Validate that the DateSpec bound via 'from' carries a valuation date.
HandleError validateBoundValuationDate(Handle from)
{
    if (handleObject<HandleType::DateSpec>(from).getValuationDate().serialNumber() == 0) {
        return HandleError(ApiErrorInvalidDate, "Valuation date not set");
    }
    return {};
}

// Standard DateSpec setter: validate then copy the DateSpec onto the request's
// dateSpec_ member. Instantiated per request handle type that exposes dateSpec_.
template <HandleType RequestT>
HandleError bindDateSpecToRequest(Handle to, Handle from, int /*setterId*/)
{
    if (HandleError error = validateBoundValuationDate(from); error.isError()) {
        return error;
    }
    handleObject<RequestT>(to).dateSpec_ = handleObject<HandleType::DateSpec>(from);
    return {};
}

// CalcGreeksFromPrices stores only the valuation date rather than the full spec.
HandleError bindDateSpecToGreeksRequest(Handle to, Handle from, int /*setterId*/)
{
    if (HandleError error = validateBoundValuationDate(from); error.isError()) {
        return error;
    }
    handleObject<HandleType::CalcGreeksFromPricesRequest>(to).valuationDate_ =
        handleObject<HandleType::DateSpec>(from).getValuationDate();
    return {};
}
...
int bindDateSpec(Handle request, Handle dateSpecOrDateSpecArray)
{
    ...
    case HandleType::GenRatePathsForMortgageValuationRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::GenRatePathsForMortgageValuationRequest>);
        break;
    case HandleType::CalcValueForMortgageRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcValueForMortgageRequest>);
        break;
    case HandleType::CalcValueForMortgageWithSofrRatesRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcValueForMortgageWithSofrRatesRequest>);
        break;
    case HandleType::AttribValueChangeByWaterfallRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::AttribValueChangeByWaterfallRequest>);
        break;
    case HandleType::GenPrimaryMortgageRatePathsRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::GenPrimaryMortgageRatePathsRequest>);
        break;
    case HandleType::CalcGreeksFromPricesRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToGreeksRequest);
        break;
    case HandleType::GenSecondaryMortgageRatePathsRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::GenSecondaryMortgageRatePathsRequest>);
        break;
    case HandleType::CalcValueForMonthEndRollRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcValueForMonthEndRollRequest>);
        break;
    case HandleType::GeneratePathForWaterfallAttributionRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::GeneratePathForWaterfallAttributionRequest>);
        break;
    case HandleType::CalcBehavioralSpeedFromPrimaryRateRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcBehavioralSpeedFromPrimaryRateRequest>);
        break;
    case HandleType::CalcValueForMortgageFromRatesRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcValueForMortgageFromRatesRequest>);
        break;
    case HandleType::CalcProfitabilityFromBehavioralSpeedsRequest:
        BIND_HANDLES(request, dateSpec, HandleBinding::DateSpec,
            bindDateSpecToRequest<HandleType::CalcProfitabilityFromBehavioralSpeedsRequest>);
        break;
    ...
    int contentLen)
{
    ...
    // Accept either input shape and deserialize into the same fully precomputed
    // rate-path set:
    //   1. A GenRatePathsForMortgageValuation response envelope (the JSON produced
    //      by ApiRequest_GenRatePathsForMortgageValuation). This mirrors the CLI
    //      parseGenRatePathsForMortgageValuationResponse used by valueMortgagesFromRates.
    //   2. A bare CalcRatePaths object (the precomputed rate paths on their own,
    //      without the response wrapper), letting callers supply/cache the rate
    //      paths directly in production rather than a full response.
    json_interpreter_options opt;
    opt.unwrap_external_object(false);
    json_parser parser(opt);
    auto var = parser.deserialize(string(content, (size_t)contentLen));

    msg::CalcRatePaths ratePaths;
    if (var.has_member("response") && var["response"].has_member("GenRatePathsForMortgageValuation")) {
        msg::GenRatePathsForMortgageValuationResponse genRatesResponse;
        var["response"]["GenRatePathsForMortgageValuation"] >> genRatesResponse;
        ratePaths = move(genRatesResponse.ratePaths_);
    }
    else {
        var >> ratePaths;
    }
    ...
    return handles->addHandleData(make_shared<HandleData>(
        HandleType::CalcRatePaths,
        move(ratePaths)));

    EXCEPTION_ERROR(NullHandle, NullHandle);
}

int bindCalcRatePaths(
    Handle request,
    Handle ratePaths)
{
    TRY
    ...
    CHECK_INIT;
    CHECK_HANDLE(request, REQUEST_HANDLES);
    CHECK_HANDLE(ratePaths, HandleType::CalcRatePaths);
    READ_LOCK_HANDLE(request)
    READ_LOCK_HANDLE(ratePaths)

    if (handleData(request).getType() == HandleType::CalcValueForMortgageFromRatesRequest)
    {
        BIND_HANDLES(request, ratePaths, HandleBinding::OverrideRatePaths,
            [](Handle to, Handle from, int setterId)->HandleError {
                auto& req = handleObject<HandleType::CalcValueForMortgageFromRatesRequest>(to);
                const auto& paths = handleObject<HandleType::CalcRatePaths>(from);
