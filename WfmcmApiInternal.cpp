...
{
	scoped_lock<recursive_mutex> lock(m_);
	//NOTE: must return a copy. Returning move(error_) cleared the stored error
	//as a side effect, so hasHandleError() wiped the error before
	//getHandleError() could report it.
	return error_;
}
